...
class resultClass:
    def __init__(self,root):
        ...
        #====title=====
        title=Label(self.root,text="Add Student Results",font=("goudy old style",20,"bold"),bg="orange",fg="#262626").place(x=10,y=15,width=1080,height=50)
        ...
        lbl_marks=Label(self.root,text="Marks Obtaines",font=("goudy old style",20,'bold'),bg='white').place(x=50,y=280)
        ...
        self.txt_student.place(x=280,y=105,width=180)
        self.txt_student.set("Select")
        btn_search=Button(self.root,text="Search",font=("goudy old style",15,'bold'),bg="#03a9f4",fg="white",cursor="hand2",command=self.search).place(x=470,y=105,width=110,height=28)
        ...
        txt_marks=Entry(self.root,textvariable=self.var_marks,font=("goudy old style",20,'bold'),bg='lightyellow').place(x=280,y=280,width=300)
        txt_full_marks=Entry(self.root,textvariable=self.var_full_marks,font=("goudy old style",20,'bold'),bg='lightyellow').place(x=280,y=340,width=300)
        ...
        btn_clear=Button(self.root,text="Clear",font=("times new roman",15),bg="lightgray",activebackground="lightgray",cursor="hand2",command=self.clear).place(x=440,y=420,width=120,height=35)

        #====image=====
        self.bg_img=Image.open("images/bg2.jpg")
        self.bg_img=self.bg_img.resize((480,280))
        self.bg_img=ImageTk.PhotoImage(self.bg_img)

        self.lbl_bg=Label(self.root,image=self.bg_img).place(x=600,y=120)
